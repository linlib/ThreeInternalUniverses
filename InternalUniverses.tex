...
\begin{tabular}{|| l | l ||} 
\hline
$\texttt{D(∞\_(∞-Cat))}$  & $\texttt{D(∞\_(∞-Cat)⁄C)}$\\
\hline
$\texttt{D(∞\_(∞-Grpd))}$  & $\texttt{D(∞\_(∞-Grpd)⁄G)}$ \\
\hline
$\texttt{D(∞\_(∞-Grpd₀))}$  & $\texttt{D(∞\_(∞-Grpd₀)⁄G₀)}$ \\
\hline
\end{tabular}}
...
\hline
$\texttt{∞\_(∞-Space)}$ &  $\texttt{D(∞\_(∞-Space))}$  & $\texttt{∞\_(A${}^{\infty}$-Alg A)}$  & $\texttt{D(∞\_(A${}^{\infty}$-Alg A))}$ \\
\hline
$\texttt{∞\_(∞-Space)}$ &  $\texttt{D(∞\_(∞-Space))}$  & $\texttt{∞\_(A${}^{\infty}$-Alg A)}$  & $\texttt{D(∞\_(A${}^{\infty}$-Alg A))}$ \\
 \hline
$\texttt{∞\_(∞-Space)}$ &  $\texttt{D(∞\_(∞-Space))}$  & $\texttt{∞\_(A${}^{\infty}$-Alg A)}$  & $\texttt{D(∞\_(A${}^{\infty}$-Alg A))}$ \\
...
\begin{center}
\texttt{Plans to construct three infinity-categories}\\
\texttt{as constituents of Mathlib's simplicial sets,}\\
\texttt{and to construct the straightening and }\\
\texttt{unstraightening constructions}
\end{center}
\ \\
\ \\
...
In this repository I would like to consider the `internal universe' ∞$\_$(∞-Cat) : ∞-Cat, which is featured in straightening and unstraightening and related to the concept of an overobject classifier.\\
...
structure adjoint_system where
  Obj : category
  Pnt : Obj.Obj
  CmpObj : Obj.Obj →
        category
...
-- Ord : ((Pul C Inf (Chi C F)) → ???
-- Ovr : (C : Obj.Obj) → (F : (CmpObj C).Obj) → ((Pul C Inf (Chi C F)).Hom (???) (???) (Chi ???) = F)
...
\iffalse
If the main results concerning fibrations, inner fibrations, left fibrations?, cofibrations, 
\fi


\iffalse
A comonadic adjoint system with overobject classifier is a pullback system.
but pullback is not always homotopy pullback. 
Pullback is effective for descent in the case of fibrations.
Fibrant replacement is the solution to the problem that.
\fi


\iffalse

Something scary to resolve:

The co-liebnitz rule or "predual liebnitz rule" concerns a graded product in which the domain of a map gets degree -1. This much occurs within 

Does this infinity box have divisiblity or not? 

The point is that maps in it are itself, by which we can pull out, apply the liebnitz product, and push in.

Three conditions for the category of 

- should by some functor subsume smash product for based ∞-groupoids X : ∞-Grpd₋₁
- should by some functor subsume cartesian product of ∞-groupoids X : ∞-Grpd
